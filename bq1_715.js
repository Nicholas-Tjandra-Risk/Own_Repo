...
if ((product_type == 'CASH_LOAN' && (cash_origination == 'EXISTING') && adjusted_score < SCORE_CUT_OFF_REPEAT_ORDER_CASH_LOAN) ||
	...
	(product_type == 'CREDIT_LIMIT' && (is_offline_applicant || is_online_applicant_nearby_offline_merchant)) ||
...
if (product_type == 'CREDIT_LIMIT' && (is_offline_applicant|| is_online_applicant_nearby_offline_merchant) && is_whitelisted_profession_credit_limit) {
	...
	if (experiment_offline == 'offline_normal_set' || is_online_applicant_nearby_offline_merchant) {
		...
		*/
		...
		if ((['hcf'].contains(offline_store_category) || (is_online_applicant_nearby_offline_merchant && gps_nearby_offline_flag_treatment == 'hcf_treatment'  && score >=340))) {
			pass_bq_hcf = true;		
		}
		/*TIER 1 */
		...
		/* bypass bq1 for online application thats near mp store */
		if ((gps_nearby_offline_flag_treatment == 'mp_treatment' && is_online_applicant_nearby_offline_merchant)) {
			if(score >= 340) {
				score_cut_off_reject_reason = '';
				job_reject_reason = '';
					identity_score_cut_off_reject_reason = '';
...
if ((is_offline_applicant||is_online_applicant_nearby_offline_merchant) && is_blacklist_store) {
	pass_bq = false;
}
...
if (((is_ibox_gan_digimap) || (nonmp_good_average) || (mp_good_average) || (is_hcf_gym) || (is_online_applicant_nearby_offline_merchant && (gps_nearby_offline_flag_treatment == 'hcf_treatment' || gps_nearby_offline_flag_treatment == 'mp_treatment') )) && product_type == 'CREDIT_LIMIT' && (is_offline_applicant || is_online_applicant_nearby_offline_merchant)&& partner !='AVANTO'){
