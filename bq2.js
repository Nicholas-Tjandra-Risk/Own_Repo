...
var is_offline_applicant = offline_transaction_applied_amount > 0  || flag_treatment == 'hcf_treatment' || flag_treatment == 'mp_treatment';
...
if (product_type == 'CASH_LOAN' && is_whitelisted_ath_via_limit_user && !is_offer_calculation) {
    /* reject low score */
...
var is_new_user = ((tiket_score <= 0) &&
                    (blibli_score <= 0) &&
                    (tiket_account_age == '' || tiket_account_age == '-99') && 
                    (tiket_account_age_max == '' || tiket_account_age_max == '-99'))

var is_additional_existing = ((tiket_sum_order_amount_paid_max > 0) ||
    ...
    if (partner == 'BLIBLI' || partner == 'TIKET') {
		if (partner == 'BLIBLI'){
			if (experiment_blibli == 'blibli_normal_set'){
                ...
                if ((getcontact_verify_status == 'OK') && (identity_score < 460)) {
                    identity_score_cut_off_reject_reason = '';
                    } 
			}
		...
		if (partner == 'TIKET'){
			if (experiment_tiket == 'tiket_normal_set'){
                ...
                if (is_new_user){
                        if ((identity_score < 480) && !(is_additional_existing)){
                            identity_score_cut_off_reject_reason = 'HA04';
                        }
                }
...
if (product_type == 'CREDIT_LIMIT' && is_offline_applicant) {
    ...
    if (experiment_offline == 'offline_normal_set' ||  (flag_treatment == 'hcf_treatment' || flag_treatment == 'mp_treatment' )) { 
        ...
        */
        ...
        if (['hcf'].contains(offline_store_category) || flag_treatment == 'hcf_treatment') {
            if(reinforced_score < 380){
                score_cut_off_reject_reason = 'JA01';
            }
        }
        ...
        /*TIER 3 */
        else if ((offline_item_category=='smartphone' && ['average'].contains(offline_store_category)) || flag_treatment == 'mp_treatment' ) {
            if(reinforced_score < 407){
                score_cut_off_reject_reason = 'JA01';
            }
        } 
