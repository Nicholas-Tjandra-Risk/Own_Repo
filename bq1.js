...
if (product_type == 'CREDIT_LIMIT' && is_offline_applicant && is_whitelisted_profession_credit_limit) {
	...
	if (experiment_offline == 'offline_normal_set' || flag_treatment == 'hcf_treatment' || flag_treatment == 'mp_treatment' ) {
		...
		*/
		...
		/* bypass bq1 for online gps that receive mp treatment */
		if ((flag_treatment == 'mp_treatment')) {
			score_cut_off_reject_reason = '';
			job_reject_reason = '';
			identity_score_cut_off_reject_reason = '';
		}
...
if (((is_ibox_gan_digimap) || (nonmp_good_average) || (mp_good_average) || (is_hcf_gym) || flag_treatment == 'hcf_treatment' || flag_treatment == 'mp_treatment') && product_type == 'CREDIT_LIMIT' && is_offline_applicant && partner !='AVANTO'){
